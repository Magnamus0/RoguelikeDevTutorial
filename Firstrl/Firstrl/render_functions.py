...
def render_bar(panel,x,y,total_width,name,value,maximum,bar_colour,back_colour): #this is where we render the HP bar
    bar_width = int(float(value) / maximum * total_width)

    libtcod.console_set_default_background(panel,back_colour)
    libtcod.console_rect(panel,x,y, total_width,1,False,libtcod.BKGND_SCREEN) #this is all black library magic
...
def render_all(con, panel, entities,player, game_map, fov_map, fov_recompute, message_log, screen_width, screen_height, bar_width ,panel_height ,panel_y ,mouse, colours, game_state):
    #draaw all the tiles in tha map
    if fov_recompute:
    ...
    entities_in_render_order = sorted(entities, key=lambda x: x.render_order.value) #sort all entites in the order they should be rendered
    for entity in entities_in_render_order:
        draw_entity(con, entity, fov_map)

    libtcod.console_blit(con, 0,0, screen_width, screen_height, 0,0,0)

    libtcod.console_set_default_background(panel, libtcod.black) #set up the pen to draw the info panel
    ...
    for message in message_log.messages:
        ...
        y += 1
    #####################################################################################################################################
    render_bar(panel,1,1,bar_width, 'HP', player.fighter.hp, player.fighter.max_hp, libtcod.light_red, libtcod.darker_red) #create the health bar
    ...
    libtcod.console_blit(panel, 0,0, screen_width, panel_height, 0,0, panel_y) #display it
    ##############################################################################################################################################
